...
import { updatePlacePrivacy } from "@/redux/slices/placePrivacySlice";
...
export default function PlacePrivacySelector({ onNext, onBack, handleSaveExit }) {
  ...
  const { loading, error } = useSelector((state) => state.placePrivacy);
  ...
  const handleNext = () => {
    if (!selected) {
      toast.error('Please select a guest accommodation type');
      return;
    }
    ...
    dispatch(updatePlacePrivacy({ 
      propertyId: property.id, 
      data: { guestAccommodationType: selected }
    }))
      .unwrap()
      .then(() => {
        toast.success('Guest accommodation type updated successfully');
        onNext();
      })
      .catch((err) => {
        toast.error(err || 'Failed to update guest accommodation type');
      });
  };
