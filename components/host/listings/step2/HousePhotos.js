...
import { uploadPhotosAsync, resetPhotosState } from '@/redux/slices/photosSlice';
...
export default function HousePhotos({ onNext, onBack, handleSaveExit }) {
  ...
  const handleNext = async () => {
    ...
    try {
      const result = await dispatch(uploadPhotosAsync({
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center w-full min-h-[400px] p-6">
        ...
        {photoError && <p className="text-red-500 text-sm text-center mt-4">{photoError}</p>}
